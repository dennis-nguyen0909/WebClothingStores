...
inputFile.onchange=function(){
    profilePic.src=URL.createObjectURL(inputFile.files[0]); let reader = new FileReader();
...
buyBtns.forEach(btn => {
  btn.addEventListener('click', (event) => {
    ...
    const product = document.querySelector(`.product-item[data-product="${productId}"]`);
    const productName = product.querySelector('.product-name').textContent;
    const productSize = product.querySelector('select[name="size"]').value;
    const productQuantity = product.querySelector('input[name="quantity"]').value;
    const productPrice = product.querySelector('.product-price').textContent.split(':')[1].trim();
